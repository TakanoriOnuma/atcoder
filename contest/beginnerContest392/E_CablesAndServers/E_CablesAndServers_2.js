...
function Main(input) {
  ...
  let leaders = dsu.groups().map((group) => dsu.leader(group[0]));
  ...
  for (let i = 0; i < minOperation; i++) {
    const edge = redundantEdges[i];
    const fromLeader = dsu.leader(edge.from);
    leaderSet.delete(fromLeader);
    /** @type {number} */
    const toLeader = leaderSet.values().next().value;
    leaderSet.delete(toLeader);

    console.log(`${edge.index + 1} ${edge.to + 1} ${toLeader + 1}`);
    leaderSet.add(dsu.merge(fromLeader, toLeader));
  }
}
